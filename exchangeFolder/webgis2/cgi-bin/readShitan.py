...
sf = shapefile.Reader("./cgi-bin/shapefile/shitan.shp")
shapes = sf.shapes()
...
for shp in range(len(shapes)):
    ...
    for i in range(len(shap.points)):
        polyline_str=polyline_str+'''
            points_st%d.push(new T.LngLat(%f, %f));
...
for num in range(len(shapes)):
	finally_str=finally_str+'''
	    var polygon_st%d = new T.Polygon(points_st%d,{
	                color: "blue", weight: 3, opacity: 0.5, fillColor: "yellow", fillOpacity: 0.5
	            });
